...
export default class WorkbenchPlugin extends Vue {
    ...
    loadPlugin() {
        ...
        reader.onload = async (e: any) => {
            ...
            let res: any = {}
            if (type === 'py') {
                res = await this.pluginStore.loadPythonPlugin({
                    graph: this.graphName,
                    data: { name: name, description: description, code_base64: [code_base64], file_name: [name + '.py'], read_only: readOnly, code_type: type }
                })
            } else {
                res = await this.pluginStore.loadCppPlugin({
                    graph: this.graphName,
                    data: { name: name, description: description, code_base64: [code_base64], file_name: [name + '.cpp'], read_only: readOnly, code_type: type }
                })
            }
            if (res.status === 200) {
                this.$message({
